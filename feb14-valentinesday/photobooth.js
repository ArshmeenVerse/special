...
class PhotoBooth {
    ...
    generateStrip() {
        ...
        // Background - Parchment / Warm White to match letter card
        const gradient = ctx.createLinearGradient(0, 0, 0, stripHeight);
        gradient.addColorStop(0, '#fffdf5');
        gradient.addColorStop(1, '#f7f4e9');
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, stripWidth, stripHeight);

        // Decoration - Gold Border
        ctx.strokeStyle = "rgba(201, 168, 76, 0.5)"; // Antique Gold
        ctx.lineWidth = 4;
        ...
        Promise.all(this.capturedImages.map(loadImage)).then(images => {
            ...
            // Footer Text
            ctx.fillStyle = "#2c0e16"; // Dark Burgundy/Black
            ...
            const isValentine = true; // Default for this file

            // Title
            ctx.font = "bold 40px 'Cinzel', serif";
            let titleText = "Valentine's Day ❤️";

            if (isTeddyDay) {
                titleText = "Teddy Day 🧸";
            } else if (isProposeDay) {
                titleText = "Propose Day 💍";
            }

            ctx.fillStyle = "#5c1a2e"; // Deep Burgundy
            ctx.fillText(titleText, centerX, footerStart);

            // Subtitle
            ctx.font = "50px 'Dancing Script', cursive";
            ctx.fillStyle = "#c9a84c"; // Gold
            ctx.fillText("My Forever & Always", centerX, footerStart + 70);
